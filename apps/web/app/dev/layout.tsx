...
// ship, since map/page.tsx renders the 404 in production and the build then
// removes the exported /dev tree. What it does carry, it carries for a
// reason, and each is noted below. No title: which of its two faces the route
// is wearing is map/page.tsx's own branch to make, so the title is stated
// there beside it.
...
export default function DevLayout({ children }: { children: React.ReactNode }) {
  return (
    // lang="sl", because what this tree renders in a production build is the
    // Slovenian 404 that map/page.tsx renders, removed from the export once
    // the build is done. The gallery's own headings are English, but they are
    // dev furniture and never leave a dev machine.
    //
    ...
    // The production render is light on a dark OS, unlike the site's own 404,
    // and nobody sees it: the build removes it from the export.
    //
