...
export function FilterCardSection({
  ...
  /** The sheet columns exist to fit several short labels side by side. A
      section whose labels are whole phrases asks for fewer, so one of them is
      a readable tile rather than a third of a row it cannot be read in. */
  sheetColumns?: 1 | 2 | 3;
  children: ReactNode;
      ...
      <CollapsibleBody collapse={collapse}>
        ...
        <LazyMotion features={domAnimation}>
          <div
            className={cn(
              "grid gap-1.5",
              layout === "sheet" ? sheetGrid : "grid-cols-1",
            )}
          >
            {children}
          </div>
