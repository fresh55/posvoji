  ...
  it("shows the distance to each shelter once an origin exists", async () => {
    ...
    const rowText = () =>
      Array.from(screen.getByRole("dialog").querySelectorAll("button"))
        ...
        .filter((text) => /^(Sever|Jug)/.test(text))
        .join("|");
...
describe("LocationPicker merged field", () => {
  ...
  it("sorts the whole list from a place without filtering it", async () => {
    ...
    expect(rowOrder()).toEqual(["sever", "jug"]);
    // The place is its chip's now, and the field a search box again: left in
    // both, the town read twice, two X's apart.
  ...
  it("keeps city-name matches until the nearby suggestion is chosen", async () => {
    ...
    // Typing the chosen place again is a name the chip already answers: the
    // list stays whole and the place is not offered a second time.
    await type(input, "Ljubljana");
  ...
  it("focuses the top name match on Enter without selecting it", async () => {
    ...
    expect(document.activeElement?.textContent).toContain("Sever");
    expect(
      screen
        .getByRole("dialog")
  ...
  it("walks into the list on ArrowDown, in either mode", async () => {
    ...
    input.focus();
    fireEvent.keyDown(input, { key: "ArrowDown" });
    expect(document.activeElement?.textContent).toContain("Jug");
  });
...
describe("LocationPicker responsive body", () => {
  ...
  it("gives map and list their own phone view above the same footer", async () => {
    ...
    expect(stage.className).toContain("picker-stacked:hidden");
    expect(panel.className).not.toContain("picker-stacked:hidden");
    ...
    expect(stage.className).not.toContain("picker-stacked:hidden");
    expect(panel.className).toContain("picker-stacked:hidden");
...
describe("LocationPicker off-site shelters", () => {
  it("links a shelter with no animals to its page", async () => {
    ...
    const link = screen.getByRole("link", { name: /Vzhod/ });
  ...
  it("folds the group shut on arrival, and opens it on a press", async () => {
    ...
    expect(screen.queryByRole("link", { name: /Vzhod/ })).toBeNull();

    openOffGroup();

    expect(screen.getByRole("link", { name: /Vzhod/ })).toBeTruthy();
  });
...
describe("LocationPicker search keys", () => {
  ...
  it("reaches an off-site row from the search when only that row matches", async () => {
    ...
    fireEvent.keyDown(input, { key: "ArrowDown" });
    expect(document.activeElement?.textContent).toContain("Vzhod");
  });
  ...
  it("reaches the off-site row once the group is open", async () => {
    ...
    expect(document.activeElement).toBe(
      screen.getByRole("link", { name: /Vzhod/ }),
    );
  });
});

// The shelters with nothing listed are rows in the list's folded group and
// nothing on the map: a mark there could not be picked, and it cost a legend
// row to explain.
describe("LocationPicker off-site shelters on the map", () => {
  it("leaves them off the plate and keeps them in the list", async () => {
    await openPicker({ offSite });
    const dialog = screen.getByRole("dialog");

    expect(dialog.querySelector('[data-marker-key*="celje" i]')).toBeNull();
    expect(dialog.querySelector("[data-marker-empty]")).toBeNull();
...
describe("LocationPicker map caption", () => {
  // Two shelters in one region, so picking one of them leaves that region
  // between states, and picking both picks it whole.
  const shared = [
  ...
  // Everything the stage says outside the plate itself.
  function captionText() {
    const stage = screen
      .getByRole("dialog")
      .querySelector<HTMLElement>("[data-map-stage]")!;
    const copy = stage.cloneNode(true) as HTMLElement;
    copy.querySelectorAll("svg").forEach((plate) => plate.remove());
    return copy.textContent;
  }

  it("draws no legend, so nothing under the map changes with a pick", () => {
    renderWith([]);
    const atRest = captionText();

    for (const selected of [["jug"], ["jug", "sever"]]) {
      cleanup();
      renderWith(selected);
      // From lg the map takes the height the caption leaves, so a caption
      // that grew a row on a pick shrank the country under the pointer.
      expect(captionText()).toBe(atRest);
    }
  });

  it("fills every region with a shelter at one flat step", async () => {
    await openPicker();

    const live = Array.from(
      screen
        .getByRole("dialog")
        .querySelectorAll("[data-region-density]"),
    ).map((region) => region.getAttribute("data-region-density"));
    expect(live.length).toBeGreaterThan(0);
    expect(new Set(live)).toEqual(new Set(["0"]));
  });
});
...
describe("LocationPicker map picking", () => {
  ...
  it("selects a region and reveals the first matching shelter", () => {
    ...
    expect(
      screen.getByRole("button", {
        name: "Odstrani zavetišče: Zavetišče Sever",
      }),
    ).toBeTruthy();
  });
  ...
  it("drops an already-picked shelter on a single click", () => {
    ...
    expect(onToggleMany).toHaveBeenCalledWith(["sever"]);
    expect(row(/^Sever/)?.getAttribute("aria-pressed")).toBe("false");
  });
  ...
  it("drops just its own shelter when a member of a picked region is clicked", () => {
    ...
    expect(onToggleMany).toHaveBeenCalledWith(["sever"]);
    expect(row(/^Sever/)?.getAttribute("aria-pressed")).toBe("false");
    expect(row(/^Vzhod/)?.getAttribute("aria-pressed")).toBe("true");
  });
  ...
  it("picks a region on one click and drops it on the next", () => {
    ...
    expect(row(/^Sever/)?.getAttribute("aria-pressed")).toBe("false");
    expect(row(/^Vzhod/)?.getAttribute("aria-pressed")).toBe("false");
  });

  it("opens no details when a region takes several shelters at once", () => {
    openMap({
      options: [
        ...options,
        { value: "vzhod", label: "Zavetišče Vzhod", city: "Ptuj" },
    ...
    expect(row(/^Sever/)?.getAttribute("aria-pressed")).toBe("true");
    expect(row(/^Vzhod/)?.getAttribute("aria-pressed")).toBe("true");
    // Opening the first of them opened whichever one sorted first and pushed
    // the rest of the same pick below the list's edge. A single shelter still
    // opens (the region test above), because there it is the whole answer.
  ...
  it("leaves open details standing when the whole selection is cleared", () => {
    openMap({ selected: ["sever", "jug"] });

    fireEvent.click(info("Zavetišče Sever"));
    expect(expanded()).toBe("sever");

    // "Počisti vse" in the footer's summary empties the selection. The card
    // this replaces folded with it, because it was an answer about something
    // picked; open details are an answer about a shelter, picked or not, and
  ...
  it("lets Enter focus a search match without changing selection or details", () => {
    ...
    expect(document.activeElement).toBe(row(/^Sever/));
    expect(row(/^Sever/)?.getAttribute("aria-pressed")).toBe("false");
    expect(expanded()).toBeNull();
  });
...
describe("LocationPicker map and list views", () => {
  ...
  const panel = () =>
    dialog().querySelector<HTMLElement>("[data-picker-sheet]")!;

  it("keeps the list beside the map when split, with nothing to fold it away", async () => {
    await openPicker();

    // The map and the list divide the body on a desktop and on a phone held
    // sideways (picker-split in globals.css). There was a control that folded
    // the list to a rail, and it bought the map no pixels: the plate is bound
    // by its height there, so a wider column only re-centred it.
    // One width, set where both read it: the list takes 24rem or less, and
    // the map everything the list does not. On a short screen the map keeps
    // only what its height fills.
    const body = dialog().querySelector("[data-picker-stage]")!.className;
    expect(body).toContain("[--picker-list-w:min(24rem,calc(100%_-_41rem))]");
    expect(body).toContain("short:[--picker-list-w:min(24rem,calc(100%_-_22rem))]");
    expect(stage().className).toContain("picker-split:right-(--picker-list-w)");
    expect(panel().className).toContain("picker-split:w-(--picker-list-w)");
    // And the switch that takes turns is gone wherever they stand together.
    expect(
      dialog().querySelector("[data-picker-view-switch]")!.className,
    ).toContain("picker-split:hidden");
    expect(screen.getByLabelText("Kraj, pošta ali zavetišče")).toBeTruthy();
    expect(dialog().querySelector("[data-picker-rail]")).toBeNull();
    expect(screen.queryByRole("button", { name: "Skrij seznam" })).toBeNull();
  });
  ...
  it("keeps selected shelter names in the footer when switching views", async () => {
    ...
    // The short name the rows print, with the full one kept in the label and
    // the title.
    expect(chip.textContent).toBe("Jug");
    expect(chip.getAttribute("title")).toBe("Zavetišče Jug");
  });

  it("stands the credit in the plate's corner from lg and under the plate below it", async () => {
    await openPicker();

    const [corner, sideways, line] = Array.from(
      stage().querySelectorAll<HTMLElement>("[data-slot='map-attribution']"),
    );
    // A phone held sideways: in the map column's own corner, the empty ground
    // beside a plate bound by the height, not in a column of its own.
    expect(sideways.parentElement).toBe(stage().querySelector("svg")!.parentElement!.parentElement);
    expect(sideways.className).toContain("absolute");
    // From lg: inside the plate's own box, beside the map, pinned to its
    // corner, so it costs the country none of its height.
    const plateBox = stage().querySelector("svg")!.parentElement!;
    expect(corner.parentElement).toBe(plateBox);
    expect(corner.className).toContain("absolute");
    expect(corner.className).toContain("hidden lg:block");
    // Stacked: the stage's own last row, where the map is sized by the width
    // and the line costs it nothing. Wherever the list stands beside the map,
    // one of the two corners above has it.
    const block = line.parentElement!;
    expect(block.parentElement).toBe(stage());
    expect(block.className).toContain("shrink-0");
    expect(block.className).toContain("picker-split:hidden");
...
describe("LocationPicker persistent footer", () => {
  it("closes from a footer outside the collapsible panel, naming the result count", async () => {
    ...
    expect(pill.closest("[data-picker-sheet]")).toBeNull();
  ...
  it("names the shelters the filters do find animals in, and adds the first", async () => {
    await openPicker({
      selected: ["sever"],
    ...
    expect(footer.querySelector("[data-picker-zero]")?.textContent).toBe(
      "0 živali. Zadetki drugje: Jug (7).",
    );
    fireEvent.click(screen.getByRole("button", { name: "Dodaj Jug" }));

    // Added beside the pick, not in place of it: the visitor's choice stays.
    expect(
      screen.getByRole("button", { name: "Odstrani zavetišče: Zavetišče Jug" }),
    ).toBeTruthy();
    expect(
      screen.getByRole("button", { name: "Pokaži izbrana zavetišča (2)" }),
    ).toBeTruthy();
  ...
  it("names three by their counts, the most first, and says how many more", async () => {
    const five = ["Arnika", "Breza", "Cvet", "Dren", "Evka"].map((name) => ({
      value: name[0].toLowerCase(),
      label: `Zavetišče ${name}`,
      city: "Ljubljana",
    }));
    await openPicker({
      options: five,
      selected: ["a"],
      counts: new Map([["b", 1], ["c", 9], ["d", 4], ["e", 2]]),
      resultCount: 0,
    });

    expect(
      screen.getByRole("dialog").querySelector("[data-picker-zero]")?.textContent,
    ).toBe("0 živali. Zadetki drugje: Cvet (9), Dren (4), Evka (2) in še 1.");
    expect(screen.getByRole("button", { name: "Dodaj Cvet" })).toBeTruthy();
  });
  ...
  it("keeps named removable selections and the result action outside either folded dock", async () => {
    ...
    fireEvent.click(dialog.querySelector("[data-picker-show-map]")!);
    expect(dialog.querySelector("[data-picker-sheet]")?.contains(footer)).toBe(false);
  ...
  it("says the count once, on the button, and not again above the list", async () => {
    ...
    const panel = screen
      .getByRole("dialog")
      .querySelector<HTMLElement>("[data-picker-sheet]")!;
  ...
  it("counts the roster, not the species facet, on the way in", async () => {
    ...
    const dialog = screen.getByRole("dialog");
    expect(
      document.querySelector("[data-picker-trigger]")?.getAttribute("aria-label"),
    ).toContain("Vsa zavetišča");
    ...
    for (const row of offSite) {
      expect(
        screen.getByRole("link", { name: new RegExp(row.label.replace(/^Zavetišče /, "")) }),
      ).toBeTruthy();
    }
  });
  ...
  it("keeps a shelter with nothing left under the filters, saying so in its row", async () => {
    ...
    expect(row.textContent).toContain("Sever");
  ...
  it("offers a named reset only while something is selected", async () => {
    ...
    // With nothing picked there is no summary to hold it.
    expect(
      screen.queryByRole("button", { name: /Pokaži izbrana zavetišča/ }),
    ).toBeNull();

    cleanup();
    await openPicker({ selected: ["jug", "sever"] });

    // In the summary that lists the selection, named for what it clears. It
    // used to be a row over the list, which appeared with the first pick and
    // pushed the list down under the pointer that made it.
    fireEvent.click(
      screen.getByRole("button", { name: "Pokaži izbrana zavetišča (2)" }),
    );
    fireEvent.click(screen.getByRole("button", { name: "Počisti vse" }));
    ...
    expect(
      screen.queryByRole("button", { name: /^Odstrani zavetišče:/ }),
    ).toBeNull();
  });
  ...
  it("keeps the title in the persistent dialog header", async () => {
    ...
    expect(title.closest("[data-picker-sheet]")).toBeNull();
  });
});
...
describe("LocationPicker attribution", () => {
  const stageCredits = () =>
    Array.from(
      screen
        .getByRole("dialog")
        .querySelectorAll<HTMLElement>("[data-map-stage] [data-slot='map-attribution']"),
    );
  ...
  it("covers the postal districts as well as the region boundaries", async () => {
    ...
    // The corner credit is names and licences only; what GURS supplied rides
    // on its link.
    const corner = stageCredits()[0];
    expect(corner.querySelector("a")?.getAttribute("title")).toBe(
      "Meje statističnih regij in poštni okoliši",
    );
  });
  ...
  it("credits the elevation model the relief is computed from", async () => {
    ...
    // Three times in the markup, one per placement, with CSS choosing one.
    const links = screen.getAllByText("Terrain Tiles");
    expect(links).toHaveLength(3);
    for (const link of links) {
      expect(link.getAttribute("href")).toContain("tilezen/joerd");
    }
    expect(screen.getAllByText(/SRTM \/ NASA/)).toHaveLength(3);
  });

  it("names the origin on the map only once there is an origin", async () => {
    const input = await openPicker();
    const plate = () => screen.getByRole("dialog").querySelector("[data-map-stage] svg")!;

    expect(plate().querySelector("[data-map-origin-name]")).toBeNull();

    await type(input, "1000");
    choosePlace();

    // The town the field resolved to, beside the dot, where a key under the
    // plate used to say "Izhodišče".
    expect(plate().querySelector("[data-map-origin-name]")?.textContent).toBe("Ljubljana");
  });
  ...
  it("keeps the GURS credit visible whenever the map is shown", async () => {
    ...
    const credits = Array.from(
      dialogNode.querySelectorAll<HTMLElement>("[data-slot='map-attribution']"),
    );
    expect(credits).toHaveLength(3);
    for (const credit of credits) expect(credit.textContent).toContain("GURS");
    // One of the three at every width: the plate's corner from lg, the map
    // column's corner on a phone held sideways, the line under the map on
    // one held upright.
    const [corner, sideways, line] = credits;
    expect(corner.className).toContain("hidden lg:block");
    expect(sideways.className).toContain("hidden max-lg:sm:short:block");
    expect(line.parentElement!.className).toContain("picker-split:hidden");
    for (const credit of credits) {
      let node: Element | null = credit.parentElement!.parentElement;
...
describe("LocationPicker marker copy", () => {
  ...
  it("tells a plate with markers on it that a shelter can be picked", async () => {
    // 2.2px to the user unit, which is the desktop plate, and a tablet's.
    plateDrawnAt(704, 462);
    await openPicker({ offSite });

    // In the footer, in the slot the first pick's chip takes. "Izberi" and
    // not "Klikni": the tablet that draws these coins is pressed with a
    // finger.
    expect(
      screen.getByText("Izberi regijo ali zavetišče").closest("[data-picker-footer]"),
    ).not.toBeNull();
    expect(screen.queryByText(/Klikni/)).toBeNull();
    const dialog = screen.getByRole("dialog");
    expect(dialog.querySelector("[data-marker-count]")).toBeTruthy();
  });
  ...
  it("asks for a region instead once the plate is too small to carry them", async () => {
    ...
    expect(screen.queryByText("Izberi regijo ali zavetišče")).toBeNull();
    const dialog = screen.getByRole("dialog");
    expect(dialog.querySelector("[data-marker-key]")).toBeNull();
  });

  it("gives the instruction's slot to the first pick's chip", async () => {
    plateDrawnAt(704, 462);
    await openPicker({ selected: ["jug"] });

    const footer = screen.getByRole("dialog").querySelector("[data-picker-footer]")!;
    expect(footer.querySelector("[data-picker-instruction]")).toBeNull();
    expect(
      screen.getByRole("button", { name: "Odstrani zavetišče: Zavetišče Jug" }),
    ).toBeTruthy();
  });

  it("says nothing about the map in the stacked list view", async () => {
    plateDrawnAt(704, 462);
    await openPicker();
    const dialog = screen.getByRole("dialog");
    const instruction = () => dialog.querySelector<HTMLElement>("[data-picker-instruction]")!;

    fireEvent.click(dialog.querySelector("[data-picker-show-list]")!);
    expect(instruction().className).toContain("picker-stacked:hidden");

    fireEvent.click(dialog.querySelector("[data-picker-show-map]")!);
    expect(instruction().className).not.toContain("picker-stacked:hidden");
  });
});
...
describe("LocationPicker sheet landing", () => {
  ...
  function onAViewport(short: boolean, mapLeads = false) {
    stubMatchMedia(
      (media) =>
        (short && media === SHORT_VIEWPORT_QUERY) ||
        (mapLeads && media === MAP_LEADS_QUERY),
    );
  }
  ...
  // A tablet held upright. It opened on its list, eleven rows in two columns
  // over 250px of empty dialog, with the map that counts every shelter a tab
  // away.
  it("lands on the map where the map writes its counts", async () => {
    onAViewport(false, true);
    await openPicker();

    expect(sheet()?.getAttribute("data-picker-sheet")).toBe("collapsed");
  });

  it("leaves that map up after a pick, which it names on the coin", async () => {
    onAViewport(false, true);
    await openPicker();

    fireEvent.click(
      screen.getByRole("button", { name: /^Osrednjeslovenska:/ }),
    );

    expect(
      screen
        .getByRole("dialog")
        .querySelector('[data-shelter-row="jug"] button[aria-pressed]')
        ?.getAttribute("aria-pressed"),
    ).toBe("true");
    expect(sheet()?.getAttribute("data-picker-sheet")).toBe("collapsed");
  });

  it("still raises the list for a pick on a phone's map", async () => {
    onAViewport(false);
    await openPicker();
    fireEvent.click(
      screen.getByRole("dialog").querySelector("[data-picker-show-map]")!,
    );
    expect(sheet()?.getAttribute("data-picker-sheet")).toBe("collapsed");
...
describe("LocationPicker audit regressions", () => {
  ...
  it("puts the locate control inside the search field", async () => {
    const input = await openPicker();
    const locate = screen.getByRole("button", { name: "Najbližje prvo" });
    // Both answers to "where" in one control: a typed place and the
    // visitor's own position.
    expect(locate.parentElement).toBe(input.parentElement);
  ...
  it("opens the summary on one shelter's removal, never on the clear-all", async () => {
    await openPicker({ selected: ["jug", "sever"] });
    fireEvent.click(
      screen.getByRole("button", { name: "Pokaži izbrana zavetišča (2)" }),
    );
  ...
  it("notes what narrows the counts only while a filter does", async () => {
    await openPicker();
    expect(
      screen.getByRole("dialog").querySelector("[data-picker-filter-context]"),
    ).toBeNull();

    cleanup();
    await openPicker({ filterSummary: "Mačke · Dodatni filtri: 2" });
    expect(
      screen.getByRole("dialog").querySelector("[data-picker-filter-context]")
        ?.textContent,
    ).toBe("Število živali upošteva filtre: Mačke · Dodatni filtri: 2");
  });
  it("hands focus to the result button before the clear-all button unmounts", async () => {
    await openPicker({ selected: ["jug", "sever"] });
    fireEvent.click(
      screen.getByRole("button", { name: "Pokaži izbrana zavetišča (2)" }),
    );
    ...
    expect(screen.queryByRole("button", { name: "Počisti vse" })).toBeNull();
    expect(
      document.activeElement?.closest("[data-picker-footer]"),
    ).toBeTruthy();
    expect(document.activeElement?.textContent).toMatch(/Pokaži|Nazaj/);
  });
  ...
  it("offers the selection popover from two shelters and lists them alphabetically", async () => {
    await openPicker({ selected: ["sever", "jug"] });
    fireEvent.click(
      screen.getByRole("button", { name: "Pokaži izbrana zavetišča (2)" }),
    );
    ...
    // One is the chip alone: a "1" beside it opened a list of that one.
    await openPicker({ selected: ["jug"] });
    expect(
      screen.queryByRole("button", { name: /^Pokaži izbrana zavetišča/ }),
    ).toBeNull();
  });
  it("hands focus to the chip when the list takes the selection down to one", async () => {
    await openPicker({ selected: ["jug", "sever"] });
    fireEvent.click(
      screen.getByRole("button", { name: "Pokaži izbrana zavetišča (2)" }),
    );
    fireEvent.click(
      within(screen.getByLabelText("Izbrano: 2")).getByRole("button", {
        name: "Odstrani zavetišče: Zavetišče Jug",
      }),
    );

    // The list and its trigger go with the second shelter; the chip that is
    // left is the whole selection, and the list's close hands it the focus.
    expect(screen.queryByLabelText("Izbrano: 2")).toBeNull();
    await waitFor(() =>
      expect(document.activeElement?.getAttribute("aria-label")).toBe(
        "Odstrani zavetišče: Zavetišče Sever",
      ),
...
describe("LocationPicker distance picks", () => {
  ...
  it("opens measuring from a place remembered from an earlier visit", async () => {
    ...
    // In its chip and nowhere else: the search box opens empty.
