...
describe("the site header", () => {
  ...
  // globals.css hangs view-transition-name: site-header on this attribute so
  // the band stands still while a navigation carries the page under it, and a
  ...
  it("marks the band the page transition pins", () => {
    ...
    const pinned = container.querySelectorAll('[data-slot="site-header"]');
    // Exactly one, and the header itself. A view-transition-name has to be
    // unique in the document or the browser drops the whole transition, so
    // the count is the assertion and not an aside to it.
