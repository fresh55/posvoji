...
// Two indicators go solid here: the destructive variant's focus ring, which
// shadcn draws at /20 and /40, and the dark invalid border it draws at /50.
// Same rule as the focus ring in globals.css, where the ring's own numbers
// live: anything saying a control is focused or invalid has to clear 3:1 by
  ...
  "group/button inline-flex shrink-0 items-center justify-center rounded-ui border border-transparent bg-clip-padding text-sm font-medium whitespace-nowrap transition-[color,background-color,border-color,box-shadow,opacity,transform] outline-none select-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring active:not-aria-[haspopup]:translate-y-px disabled:pointer-events-none disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-3 aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
  {
    variants: {
      variant: {
        ...
        outline:
          "border-control-border bg-background shadow-xs hover:bg-muted hover:text-foreground aria-expanded:bg-muted aria-expanded:text-foreground dark:bg-input/30 dark:hover:bg-input/50",
        secondary:
      ...
      size: {
        default:
          "h-9 gap-1.5 px-2.5 in-data-[slot=button-group]:rounded-ui has-data-[icon=inline-end]:pr-2 has-data-[icon=inline-start]:pl-2",
        // Contact actions can wrap at narrow widths or enlarged text sizes.
        wrap: "h-auto min-h-9 gap-1.5 whitespace-normal px-2.5 py-2 pointer-coarse:min-h-11",
        xs: "h-6 gap-1 rounded-ui px-2 text-xs in-data-[slot=button-group]:rounded-ui has-data-[icon=inline-end]:pr-1.5 has-data-[icon=inline-start]:pl-1.5 [&_svg:not([class*='size-'])]:size-3",
        sm: "h-8 gap-1 rounded-ui px-2.5 in-data-[slot=button-group]:rounded-ui has-data-[icon=inline-end]:pr-1.5 has-data-[icon=inline-start]:pl-1.5",
        lg: "h-10 gap-1.5 px-2.5 has-data-[icon=inline-end]:pr-2 has-data-[icon=inline-start]:pl-2",
        icon: "size-9",
        "icon-xs":
          "size-6 rounded-ui in-data-[slot=button-group]:rounded-ui [&_svg:not([class*='size-'])]:size-3",
        "icon-sm":
          "size-8 rounded-ui in-data-[slot=button-group]:rounded-ui",
        "icon-lg": "size-10",
      },
    },
    defaultVariants: {
