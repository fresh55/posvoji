...
There are no visible controls, captions or sounds. While the model loads, a status label at the foot of the stage says so, after half a second or at once when the poster is touched, and the pointer shows progress over it; a touch the poster took is answered with `Notice` once the cat is ready. Animation pauses offscreen and in hidden tabs, and reduced motion disables automatic animation and responses. The model and renderer load only when needed on the about page. A head preload of the model on the gate was measured and dropped: the link is bandwidth-bound, so it finished the load only 0.3s sooner on 4G while delaying hydration.
