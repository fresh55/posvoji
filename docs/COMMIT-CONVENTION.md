...
`feat` and `fix` are the two the spec defines; the rest are the ones we use
here.
...
The scope is optional but expected here. It is usually the workspace or the
shelter slug:
...
description of what broke and what to do about it. In this repo that means the
`Animal` schema, the `ProviderPolicy` shape, or the `AdoptionProvider`
interface: anything a provider outside this repo could depend on.
...
because tooling reads them. The description and body can be Slovenian or
English.
...
and has to be a valid conventional commit on its own. The commits on your
branch should follow the convention too, but the title is the one that lasts.

## Anti-patterns

- `update stuff`: no type, and no information either.
- `feat: fix the age parser`: that is a `fix`.
- `feat(web): add filters, bump deps, and fix the map`: three changes in one
  commit. Keep PRs focused; the convention only works if a commit does one
  thing.
