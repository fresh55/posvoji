...
if "${LAYOUT_V2}"; then
  ...
  echo "  !!"
  echo "  !! Then verify, before the scheduled crawl goes back on. Run one"
  echo "  !! deploy by hand once the marker exists: that deploy is the first"
  echo "  !! release with a real public/ and a private/ beside it, and until"
  echo "  !! it is there the checks below describe nothing."
  echo "  !!"
  echo "  !! Production is behind HTTP Basic Auth, so every unauthenticated"
  echo "  !! request gets a 401, including a request for a path that does not"
  echo "  !! exist. Fetching /private/dataset.published.json without"
  echo "  !! credentials and seeing a 401 says nothing about whether the file"
  echo "  !! is reachable: 401 comes back either way, and a 404 is never"
  echo "  !! reached to be seen. It is the same reason this script's own"
  echo "  !! post-flip check accepts 200 or 401. The structural checks need"
  echo "  !! no credentials and are exact; the HTTP checks need credentials"
  echo "  !! to mean anything."
  echo "  !!"
  echo "  !! Every block below runs under set -eu and asserts what it checks,"
  echo "  !! so a failed check exits non-zero and the block stops there. Each"
  echo "  !! block ends with a single pass line that is only reachable when"
  echo "  !! everything in it passed. That line is the pass, not the absence"
  echo "  !! of alarming output: a block that printed some of its output and"
  echo "  !! stopped is a failure, and so is a block that exited non-zero"
  echo "  !! after printing nothing."
  echo "  !!"
  echo "  !!   A. Structural checks, on the host:"
  echo "  !!"
  echo "  !!        ("
  echo "  !!          set -eu"
  echo "  !!          cur=${CURRENT_LINK}"
  echo "  !!          test -d \"\$cur/public\" || { echo \"public/ is not a real directory\" >&2; exit 1; }"
  echo "  !!          test ! -L \"\$cur/public\" ||"
  echo "  !!            { echo \"public/ is a symlink, so this is a v1 release\" >&2; exit 1; }"
  echo "  !!          for f in dataset.crawled.json dataset.published.json publication.json; do"
  echo "  !!            test -s \"\$cur/private/\$f\" || { echo \"missing: private/\$f\" >&2; exit 1; }"
  echo "  !!          done"
  echo "  !!          test ! -e \"\$cur/public/private\" ||"
  echo "  !!            { echo \"private/ is inside the docroot\" >&2; exit 1; }"
  echo "  !!          echo \"structural checks: OK\""
  echo "  !!        )"
  echo "  !!"
  echo "  !!      \"structural checks: OK\" is the pass. The first two checks"
  echo "  !!      are what separate a real v2 release from a self-linked v1"
  echo "  !!      one: test -s public/index.html alone follows the self-symlink"
  echo "  !!      and passes on both. It is the same assertion this script"
  echo "  !!      makes before every flip."
  echo "  !!"
  echo "  !!      The receipt names the release it was written for, so it has"
  echo "  !!      to agree with what current points at:"
  echo "  !!"
  echo "  !!        ("
  echo "  !!          set -eu"
  echo "  !!          receipt=${CURRENT_LINK}/private/publication.json"
  echo "  !!          cur=\"\$(basename \"\$(readlink ${CURRENT_LINK})\")\""
  echo "  !!          rid=\"\$(sed -n 's/.*\"releaseId\"[[:space:]]*:[[:space:]]*\"\\([^\"]*\\)\".*/\\1/p' \"\$receipt\")\""
  echo "  !!          echo \"current:   \$cur\""
  echo "  !!          echo \"releaseId: \$rid\""
  echo "  !!          test \"\$cur\" = \"\$rid\" ||"
  echo "  !!            { echo \"the receipt names \$rid, current points at \$cur\" >&2; exit 1; }"
  echo "  !!          echo \"receipt check: OK\""
  echo "  !!        )"
  echo "  !!"
  echo "  !!      The block exits non-zero when the release directory and the"
  echo "  !!      receipt inside it disagree, because that means the private"
  echo "  !!      half is not the one that deploy built."
  echo "  !!   B. Authenticated HTTP checks, through Caddy, from the host. The"
  echo "  !!      credentials go into a temporary .netrc that curl reads, so"
  echo "  !!      they stay off the command line, out of the process list and"
  echo "  !!      out of curl's own output:"
  echo "  !!"
  echo "  !!        ("
  echo "  !!          set -eu"
  echo "  !!          umask 077"
  echo "  !!          netrc=\"\$(mktemp)\""
  echo "  !!          trap 'rm -f \"\$netrc\"' EXIT"
  echo "  !!          trap 'exit 130' INT TERM"
  echo "  !!          cat >\"\$netrc\" <<'NETRC'"
  echo "  !! machine posvoji.si login BASIC_AUTH_USER password BASIC_AUTH_PASSWORD"
  echo "  !! NETRC"
  echo "  !!          for pair in \"/ 200\" \"/private/dataset.published.json 404\" \"/publication.json 404\"; do"
  echo "  !!            p=\"\${pair% *}\""
  echo "  !!            want=\"\${pair##* }\""
  echo "  !!            code=\"\$(curl -s -o /dev/null -w '%{http_code}' --netrc-file \"\$netrc\" \\"
  echo "  !!              -k --resolve posvoji.si:443:127.0.0.1 \"https://posvoji.si\$p\")\""
  echo "  !!            echo \"\$code \$p\""
  echo "  !!            test \"\$code\" != 401 ||"
  echo "  !!              { echo \"401 on \$p: the credentials did not apply, so this run is inconclusive\" >&2"
  echo "  !!                exit 1; }"
  echo "  !!            test \"\$code\" = \"\$want\" || { echo \"\$p answered \$code, expected \$want\" >&2; exit 1; }"
  echo "  !!          done"
  echo "  !!          echo \"authenticated HTTP checks: OK\""
  echo "  !!        )"
  echo "  !!"
  echo "  !!      BASIC_AUTH_USER and BASIC_AUTH_PASSWORD are placeholders for"
  echo "  !!      the real pair. umask 077 keeps the file unreadable to anyone"
  echo "  !!      else while it exists. The trap is installed straight after"
  echo "  !!      mktemp and before the credentials are written, and it is"
  echo "  !!      what removes the file on every path out of the block: the"
  echo "  !!      normal end, a failed check, and an interrupt. A plain rm -f"
  echo "  !!      at the bottom would only cover the first of those. The second"
  echo "  !!      trap is what makes an interrupt a failure: a POSIX INT"
  echo "  !!      handler that does not exit lets the shell carry on after the"
  echo "  !!      interrupted command, so without it the block could finish and"
  echo "  !!      report a pass after a Ctrl+C. A heredoc"
  echo "  !!      typed at the prompt still reaches the shell history, so"
  echo "  !!      either put the line in the file with an editor or paste the"
  echo "  !!      block with a leading space on a shell that has"
  echo "  !!      HISTCONTROL=ignorespace."
  echo "  !!"
  echo "  !!      The expected codes are asserted rather than printed for"
  echo "  !!      reading: 200 for /, 404 for /private/dataset.published.json"
  echo "  !!      and 404 for /publication.json. Anything else fails the"
  echo "  !!      block. A 401 fails it with its own message, because it means"
  echo "  !!      the credentials did not apply and the run then proves"
  echo "  !!      nothing: it is inconclusive, not a pass. Fix the credentials"
  echo "  !!      and run it again."
  echo "  !!   C. The rollback drill, once, at migration time. A rollback onto"
  echo "  !!      a self-linked v1 release is what step 1 exists for, and"
  echo "  !!      doing it is the only way to know it works. The scheduled"
  echo "  !!      task has to stay disabled for the drill: a crawl deploy"
  echo "  !!      landing in the middle of it flips current underneath you."
  echo "  !!"
  echo "  !!        ls -ld ${RELEASES_DIR}/*/public   # the v1 ones are symlinks"
  echo "  !!"
  echo "  !!        ("
  echo "  !!          set -eu"
  echo "  !!          v1=${RELEASES_DIR}/PICK-A-V1-RELEASE"
  echo "  !!          test -L \"\$v1/public\""
  echo "  !!          v2=\"\$(readlink ${CURRENT_LINK})\""
  echo "  !!          umask 077"
  echo "  !!          netrc=\"\$(mktemp)\""
  echo "  !!          cleanup() {"
  echo "  !!            ln -sfn \"\$v2\" ${CURRENT_LINK}"
  echo "  !!            chown -h ${OWNERSHIP} ${CURRENT_LINK}"
  echo "  !!            rm -f \"\$netrc\""
  echo "  !!            echo \"restored: current -> \$v2\""
  echo "  !!          }"
  ...
  echo "  !! machine posvoji.si login BASIC_AUTH_USER password BASIC_AUTH_PASSWORD"
  echo "  !! NETRC"
  echo "  !!          echo \"current: \$v2\""
  echo "  !!          ln -sfn \"\$v1\" ${CURRENT_LINK}"
  echo "  !!          chown -h ${OWNERSHIP} ${CURRENT_LINK}"
  echo "  !!          test -s ${CURRENT_LINK}/public/index.html"
  echo "  !!          code=\"\$(curl -s -o /dev/null -w '%{http_code}' --netrc-file \"\$netrc\" \\"
  echo "  !!            -k --resolve posvoji.si:443:127.0.0.1 https://posvoji.si/)\""
  echo "  !!          test \"\$code\" = 200 || { echo \"v1 release answered \$code, expected 200\" >&2; exit 1; }"
  echo "  !!          echo \"the v1 release serves through current/public\""
  echo "  !!        )"
  echo "  !!"
  echo "  !!      PICK-A-V1-RELEASE is a placeholder, and test -L \"\$v1/public\""
  echo "  !!      stops the block before it flips anything if the name was not"
  echo "  !!      replaced: ln -sfn onto a name that does not exist would leave"
  echo "  !!      current dangling and the site down."
  echo "  !!"
  echo "  !!      The restore is the trap's job, not a line at the end, and"
  echo "  !!      the trap is installed before the first flip. From the moment"
  echo "  !!      current can point at v1 there is already something that puts"
  echo "  !!      v2 back, and it runs on every exit path: the drill passing,"
  echo "  !!      a check failing, and Ctrl+C in the middle. The drill cannot"
  echo "  !!      leave production on the old release. The INT TERM trap exits"
  echo "  !!      rather than restoring by itself, so an interrupt runs the"
  echo "  !!      restore once, through EXIT, and leaves a non-zero status"
  echo "  !!      behind: an interrupted drill is a failed drill."
  echo "  !!"
  echo "  !!      The check is authenticated and has to see a 200. An"
  echo "  !!      unauthenticated curl -skI ... | head -1 would not do the"
  echo "  !!      job: it accepts the same 401 called inconclusive above, and"
  echo "  !!      the pipe makes the exit status head's, so a request that"
  echo "  !!      failed outright would not even stop the block."
  echo "  !!"
  echo "  !!      Then confirm the restore landed:"
  echo "  !!"
  echo "  !!        ("
  echo "  !!          set -eu"
  echo "  !!          umask 077"
  ...
  echo "  !!          test -d \"\$cur/public\" || { echo \"public/ is not a real directory\" >&2; exit 1; }"
  echo "  !!          test ! -L \"\$cur/public\" ||"
  echo "  !!            { echo \"current still points at the v1 release\" >&2; exit 1; }"
  echo "  !!          test -s \"\$cur/public/index.html\" ||"
  echo "  !!            { echo \"index.html is missing or empty\" >&2; exit 1; }"
  echo "  !!          code=\"\$(curl -s -o /dev/null -w '%{http_code}' --netrc-file \"\$netrc\" \\"
  echo "  !!            -k --resolve posvoji.si:443:127.0.0.1 https://posvoji.si/)\""
  echo "  !!          test \"\$code\" = 200 ||"
  echo "  !!            { echo \"the restored release answered \$code, expected 200\" >&2; exit 1; }"
  echo "  !!          echo \"restore confirmed: OK\""
  echo "  !!        )"
  echo "  !!"
  echo "  !!      current/public being a real directory and not a symlink is"
  echo "  !!      what says the v2 release is back, and the 200 says it is"
  echo "  !!      being served. ln -sfn in the drill is the same single"
  echo "  !!      operation this script uses for its own flip, so the drill"
  echo "  !!      runs the ordinary path rather than a special one."
  echo "  !!"
  echo "  !! Re-enable the scheduled task only after the structural checks,"
  echo "  !! the authenticated checks and the drill have all passed:"
  echo "  !!"
  echo "  !!   Enable-ScheduledTask -TaskName PosvojiCrawlDeploy -TaskPath \\Posvoji\\"
  echo
fi
