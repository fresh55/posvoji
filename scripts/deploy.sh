...
if "${LAYOUT_V2}"; then
  ...
  echo "  !!          test -d \"\$cur/public\" && test ! -L \"\$cur/public\" ||"
  echo "  !!            echo \"public/ is not a real directory\""
  echo "  !!          for f in dataset.crawled.json dataset.published.json publication.json; do"
  echo "  !!            test -s \"\$cur/private/\$f\" || echo \"missing: private/\$f\""
  echo "  !!          done"
  echo "  !!          test ! -e \"\$cur/public/private\" || echo \"private/ is inside the docroot\""
  echo "  !!        )"
  echo "  !!"
  echo "  !!      Silence is the pass. The first check is what separates a"
  echo "  !!      real v2 release from a self-linked v1 one: test -s"
  echo "  !!      public/index.html alone follows the self-symlink and passes"
  echo "  !!      on both. It is the same assertion this script makes before"
  echo "  !!      every flip."
  echo "  !!"
  ...
  echo "  !!          [ \"\$cur\" = \"\$rid\" ] && echo match || echo MISMATCH"
  echo "  !!        )"
  echo "  !!"
  echo "  !!      A mismatch means the release directory and the receipt"
  echo "  !!      inside it disagree, so the private half is not the one that"
  echo "  !!      deploy built."
  ...
  echo "  !! NETRC"
  echo "  !!          for p in / /private/dataset.published.json /publication.json; do"
  ...
  echo "  !!      else while it exists, and rm -f keeps it from outliving the"
  echo "  !!      check. A heredoc typed at the prompt still reaches the shell"
  echo "  !!      history, so either put the line in the file with an editor"
  echo "  !!      or paste the block with a leading space on a shell that has"
  echo "  !!      HISTCONTROL=ignorespace."
  echo "  !!"
  echo "  !!      Expected: 200 /, then 404 /private/dataset.published.json"
  echo "  !!      and 404 /publication.json. A 401 on any of the three means"
  echo "  !!      the credentials did not apply, and then the run proves"
  ...
  echo "  !!          test -s ${CURRENT_LINK}/public/index.html"
  echo "  !!          curl -skI --resolve posvoji.si:443:127.0.0.1 https://posvoji.si/ | head -1"
  ...
  echo "  !!"
  echo "  !!      Both test -s checks pass and both curls answer 200 or 401,"
  echo "  !!      the v1 release through its self-symlink and the v2 one"
  echo "  !!      through its real public/. ln -sfn here is the same single"
