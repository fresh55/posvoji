...
import {
  runBash,
  shellPath as posix,
  shellQuote as shQuote,
} from "./operation-shell.mjs";
...
try {
  ...
  writeFileSync(
    join(site, "_posvoji", "status.json"),
    '{"releaseId":"expected"}\n',
  );
  writeFileSync(
    join(bin, "curl"),
...
`,
  ...
  for (const scenario of [
    ...
    const run = runBash(["-c", command], {
      encoding: "utf8",
      env: { ...process.env, SCENARIO: scenario, EXPECTED: posix(site) },
    });
    assert.equal(
      run.status === 0,
      scenario === "healthy",
      `${scenario}: ${run.stdout}${run.stderr}`,
    );
  }
